...
async def db_start():
    global db, cur
    cur.execute("CREATE TABLE IF NOT EXISTS person(user_id TEXT PRIMARY KEY, name TEXT, mail TEXT)")
    db.commit()
...
async def edit_profile(data, user_id):
    cur.execute(f"UPDATE person SET name = ?, mail = ? WHERE user_id == ?", (data[0], data[1], user_id))
    db.commit()
